...
function handleClick(clickedButtonEvent) {
    ...
    let findButtonText = card.button.innerText;
    
    if (findButtonText == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (findButtonText == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function hideMoreText(card) {
    ...
    let findDesc = card.wrap.querySelector('.desc');
    findDesc.remove();
...
function showMoreText(card) {
    ...
    let cardText = 'Подробное описание товара';
